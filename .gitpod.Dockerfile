...
RUN echo "substituters = https://cache.nixos.org/ https://nixcache.reflex-frp.org" >> /etc/nix/nix.conf
RUN echo "trusted-public-keys = ryantrinkle.com-1:JJiAKaRv9mWgpVAz8dwewnZe0AzzEAzPkagE9SP5NWI= cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY=" >> /etc/nix/nix.conf
